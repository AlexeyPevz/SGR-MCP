...
class SGRServer:
	...
	def _register_handlers(self):
			...
			tools = [
				...
				Tool(
					...
					inputSchema={
						"type": "object",
						"properties": {
							"agent_endpoint": {"type": "string", "description": "The endpoint or identifier of the agent"},
							"agent_request": {"type": "object", "description": "The request payload for the agent"},
							"sgr_config": {
								"type": "object",
				...
				Tool(
					...
					inputSchema={
						"type": "object",
						"properties": {
							"examples": {"type": "array", "description": "Example inputs and reasoning outputs", "minItems": 3},
							"task_type": {"type": "string", "description": "Name for the new schema"},
						},
		...
		async def call_tool(name: str, arguments: Dict[str, Any]) -> CallToolResult:
			...
			try:
				if name == "apply_sgr":
					result = await apply_sgr_tool(arguments, self.llm_client, self.cache_manager, self.telemetry)
				elif name == "wrap_agent_call":
					result = await wrap_agent_call_tool(arguments, self.llm_client, self.cache_manager, self.telemetry)
				elif name == "enhance_prompt_with_sgr":
					result = await enhance_prompt_tool(arguments, self.llm_client, self.cache_manager)
				...
				return CallToolResult(
					content=[TextContent(type="text", text=json.dumps({"error": str(e), "type": type(e).__name__}))]
				)
		...
		async def list_resources() -> ListResourcesResult:
			...
			resources = [
				Resource(uri="sgr://schemas", name="schema_library", description="Available SGR schemas", mimeType="application/json"),
				Resource(uri="sgr://policy", name="policy", description="Current routing and budget policy", mimeType="application/yaml"),
				Resource(uri="sgr://traces", name="traces", description="Recent reasoning traces", mimeType="application/json"),
			]
