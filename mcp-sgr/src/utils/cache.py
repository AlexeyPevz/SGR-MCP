...
class CacheManager:
        ...
        try:
            ...
            await self._cache_db.execute(
                """INSERT OR REPLACE INTO cache_entries 
                   ...
                   VALUES (?, ?, ?, ?, 0)""",
                (key, json.dumps(value), datetime.utcnow(), expires_at)
            )
            await self._cache_db.commit()
    ...
    async def _clean_old_traces(self):
        ...
        try:
            cutoff_date = datetime.utcnow() - timedelta(days=self.trace_retention_days)
            
            await self._trace_db.execute(
                "DELETE FROM trace_entries WHERE created_at < ?",
                (cutoff_date,)
            )
            await self._trace_db.commit()
    ...
    async def get_cache_stats(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if stats["total_hits"] > 0:
                stats["hit_rate"] = stats["total_hits"] / (stats["total_hits"] + stats["total_entries"])
            else:
                stats["hit_rate"] = 0.0
