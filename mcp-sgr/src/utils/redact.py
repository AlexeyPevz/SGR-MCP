...
class PIIRedactor:
    ...
    PATTERNS = {
        ...
        "phone": RedactionPattern(
            ...
            pattern=re.compile(
                r"(?:\+?\d{1,3}[-.\s]?)?(?:\(\d{3}\)|\d{3})?[-.\s]?\d{3}[-.\s]?\d{4}|\b\d{3}[-.\s]?\d{4}\b"
            ),
            replacement="[PHONE]",
            hash_it=True,
        ),
        ...
        "api_key": RedactionPattern(
            name="api_key",
            pattern=re.compile(
                r'\b(?:api[_-]?key|apikey|api[_-]?secret|access[_-]?token)["\']?\s*[:=]\s*["\']?([A-Za-z0-9+/=_-]{20,})["\']?\b',
                re.IGNORECASE,
            ),
            replacement="[API_KEY]",
            hash_it=False,
    ...
    def __init__(self, enabled: bool = True, patterns: Optional[List[str]] = None):
        ...
        if patterns:
            self.active_patterns = {
                name: pattern for name, pattern in self.PATTERNS.items() if name in patterns
            }
        else:
    ...
    def redact_text(self, text: str) -> Tuple[str, Dict[str, int]]:
        ...
        for name, pattern in self.active_patterns.items():
            ...
            def _repl(m: Match[str]) -> str:
                ...
                if pattern.hash_it:
                    # Use SHA-256 to avoid weak hash warnings; keep short suffix for readability
                    hash_suffix = hashlib.sha256(m.group(0).encode()).hexdigest()[:8]
    ...
    def add_custom_pattern(
        self, name: str, pattern: str, replacement: str = "[CUSTOM]", hash_it: bool = False
    ) -> None:
        ...
        try:
            ...
            self.active_patterns[name] = RedactionPattern(
                name=name, pattern=compiled, replacement=replacement, hash_it=hash_it
            )
