...
@cli.command()
@click.argument("task")
@click.option("--schema", "-s", default="auto", 
              type=click.Choice(["auto", "analysis", "planning", "decision", 
                                "code_generation", "summarization"]),
              help="Schema type to use")
@click.option("--budget", "-b", default="lite",
    ...
    async def show_traces():
        ...
        try:
            ...
            for trace in traces:
                ...
                if 'task' in args:
                    task_preview = args['task'][:100] + "..." if len(args['task']) > 100 else args['task']
