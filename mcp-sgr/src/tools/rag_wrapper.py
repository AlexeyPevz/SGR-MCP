...
async def rag_enhanced_search(
    ...
    try:
        ...
        pre_analysis = await _perform_pre_analysis(query, rag_config, llm_client)

        # Phase 2: Enhanced retrieval
        logger.info("Performing enhanced retrieval")
        retrieval_results = await _perform_retrieval(pre_analysis, retrieval_fn, rag_config)

        # Phase 3: Generate answer with sources
        logger.info("Generating answer from sources")
        answer = await _generate_answer(query, retrieval_results, llm_client)

        # Phase 4: Post-retrieval validation
        logger.info("Validating results")
        validation = await _perform_validation(query, retrieval_results, answer, llm_client)
...
async def _perform_pre_analysis(
    query: str, config: Dict[str, Any], llm_client: LLMClient
) -> Dict[str, Any]:
    ...
    _ = await llm_client.generate(prompt, temperature=0.1, max_tokens=1000)
...
async def _perform_retrieval(
    pre_analysis: Dict[str, Any], retrieval_fn: Optional[Any], config: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    return {"documents": ranked_docs[: config.get("top_k", 5)], "iterations": len(queries)}


async def _generate_answer(
    query: str, retrieval_results: Dict[str, Any], llm_client: LLMClient
) -> str:
    """Generate answer from retrieved sources."""

    context = "\n\n".join(
        [f"Source {i+1}: {doc['content']}" for i, doc in enumerate(retrieval_results["documents"])]
    )
...
async def _perform_validation(
    query: str, retrieval_results: Dict[str, Any], answer: str, llm_client: LLMClient
) -> Dict[str, Any]:
    """Validate retrieval results and answer."""

    schema = RAGValidationSchema()

    context = {"documents": retrieval_results["documents"], "answer": answer}

    prompt = schema.generate_prompt(query, context)

    _ = await llm_client.generate(prompt, temperature=0.1, max_tokens=1500)
    ...
    validation = {
        ...
        "claim_to_source_map": [],
        "coverage_assessment": {"coverage_score": 0.75, "completeness": "partial"},
        "missing_evidence": [],
        "confidence_factors": {"overall_confidence": 0.72},
    }
...
def _rerank_documents(
    docs: List[Dict[str, Any]], pre_analysis: Dict[str, Any]
) -> List[Dict[str, Any]]:
