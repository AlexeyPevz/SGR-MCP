...
async def wrap_agent_call_tool(
    ...
    try:
        ...
        if pre_analysis:
            ...
            enhanced_request = await _enhance_request(agent_request, pre_reasoning, llm_client)
        else:
        ...
        if post_analysis:
            ...
            quality_metrics = await _calculate_quality_metrics(
                agent_request, agent_response, post_reasoning, llm_client
            )
            ...
            if quality_metrics.get("confidence", 1.0) < 0.7 or include_alternatives:
                suggestions = await _generate_suggestions(
                    agent_request, agent_response, post_reasoning, quality_metrics, llm_client
                )
...
4. Risk factors"""
    ...
    result = await apply_sgr_tool(
        {
            "task": task,
            "context": {"agent_type": agent_type, "request_size": len(json.dumps(agent_request))},
...
async def _enhance_request(
    original_request: Dict[str, Any], pre_reasoning: Dict[str, Any], llm_client: LLMClient
) -> Dict[str, Any]:
